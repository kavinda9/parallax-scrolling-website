...
const CloudRenderItem = memo(function CloudRenderItem({
  cloud,
  videoTime,
  getTransform,
}) {
  ...
  if (videoTime >= cloudDisappearTime) {
    const fadeProgress = (videoTime - cloudDisappearTime) / disappearDuration;
...
function CloudsTransition({ videoTime }) {
  // Cloud configuration - memoized to prevent recreating on every render
